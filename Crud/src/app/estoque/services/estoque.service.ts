...
export class EstoqueService {
  private readonly API = 'https://receitai-api.herokuapp.com/api/produtos';
  page!: Page;
  ...
  getCategorias() {
    return this.httpClient
      .get<Page>(`${this.API}` + `/findCategoriaAsc`)
      .pipe(first(), delay(500));
  }
  getFornecedores() {
    return this.httpClient
      .get<Page>(`${this.API}` + `/findFornecedorDistinctByQtdDisponivel`)
      .pipe(first(), delay(500));
  }
  ...
  getProdutosParaReceita() {
    return this.httpClient
      .get<Produto[]>(`${this.API}` + `/getProdutos`)
      .pipe(first(), delay(500));
  }
}
